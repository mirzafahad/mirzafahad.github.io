...
**Epoch:** Training runs this process (adjusting neurons’ weights and biases) on the full dataset multiple times, and each full run-through is known as an epoch. Don't use a high number of epochs though. Otherwise, the model will overfit their training data.
