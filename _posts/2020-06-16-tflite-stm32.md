...
{: .box-note}  
**Note:** If the STM32 application becomes too much for you I would suggest you try one of their Arduino examples. Those are pretty straight forward. I tried the SparkFun Edge example too. Not worth the hassle at the time of writing.  
...
# Hello, World!  
LED blinking is the de-facto 'Hello, World!' example for Embedded System. So for Neural Network in Embedded System example I will choose an LED. But instead of blinking we will fade in - fade out the LED. Now, if you are already familiar with Embedded System you know that you can do that using PWM. But instead we will apply a Sine Wave shaped analog signal on a pin where the LED is connected.
...
Also, as the development board that I am using has an LCD display, we can plot the sine wave on there too. In a nutshell, I will show you:  
...
![model block diagram](/img/tflite/code_section.png){: .center-block :}  
